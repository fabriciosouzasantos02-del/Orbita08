...
function scanDirectory(dir: string) {
  ...
  for (const file of files) {
    ...
    if (stat.isDirectory()) {
      ...
      if (['translations.ts', 'check-translations.ts', 'check-numerology-translations.ts', 'validate-translations.ts', 'audit-i18n.ts', 'autoAuditPatch.ts', 'translationPatch.ts', 'data.ts'].includes(file)) continue;
      ...
      lines.forEach((line, index) => {
        ...
        for (const match of rawTextMatches) {
          ...
          if (!isLegacyTranslationKey(matchedText)) continue; // Allow semantic key calls
