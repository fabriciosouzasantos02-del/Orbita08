...
export default function TarotTab({ lang }: TarotTabProps) {
  ...
  const handleGetInterpretation = async () => {
    ...
    try {
      ...
      console.error(err);
      setInterpretation(t("tarot.ui.aiError"));
    } finally {
      setLoadingAI(false);
    }
  };
  ...
  const getSpreadName = (pos: 'passado' | 'presente' | 'futuro') => {
    if (pos === 'passado') return t("tarot.spread.past");
    if (pos === 'presente') return t("tarot.spread.present");
    return t("tarot.spread.future");
  };
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="bg-white border border-neutral-200/90 rounded-2xl p-6 shadow-sm">
        <h2 className="text-xl font-display font-semibold text-neutral-900">{t("tarot.ui.title")}</h2>
        <p className="text-neutral-500 text-xs sm:text-sm mt-1 max-w-2xl leading-relaxed">
          {t("tarot.ui.description")}
        </p>
      </div>
        ...
        <div className="space-y-1.5">
          ...
          <div className="flex flex-col sm:flex-row gap-3">
            <input
              ...
              placeholder={t("tarot.ui.placeholder")}
            ...
            >
              {isShuffling ? (
                <>
                  ...
                  <span>{t("tarot.ui.shuffling")}</span>
                </>
              ) : (
                <>
                  <Sparkles className="w-4 h-4 text-amber-400" />
                  <span>{t("tarot.ui.drawCards")}</span>
                </>
              )}
            </button>
          </div>
                  ...
                  >
                    ...
                    <div className="relative w-36 h-56 bg-gradient-to-b from-indigo-900/10 to-indigo-950/25 border-4 border-indigo-900/10 rounded-2xl p-3 flex flex-col justify-between items-center shadow-md select-none transform transition overflow-hidden">
                      <div className="text-[10px] uppercase font-bold tracking-widest text-indigo-300">
                        {t("tarot.ui.arcanaLabel")} {drawn.card.arcana}
                      </div>
                      ...
                      <div className="w-full text-center">
                        <span className="block text-[10px] text-neutral-400 font-medium">{t("tarot.ui.positionLabel")}</span>
                        <span className="block text-[10px] font-bold text-neutral-600">
                          {drawn.isReversed ? t("tarot.ui.reversedPosition") : t("tarot.ui.uprightPosition")}
                        </span>
                      </div>
                    </div>
                ...
                >
                  {loadingAI ? (
                    <>
                      ...
                      <span>{t("tarot.ui.aiInvocating")}</span>
                    </>
                  ) : (
                    <>
                      <Sparkles className="w-3.5 h-3.5 text-amber-500 animate-pulse" />
                      <span>{t("tarot.ui.aiInterpretButton")}</span>
                    </>
                  )}
                </button>
              </div>
